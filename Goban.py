...
class Board:
    ...
    def __str__(self):
        ...
        for i, c in enumerate(self._board):
            toreturn += self._piece2str(
                c) + " "  # +'('+str(i)+":"+str(self._stringUnionFind[i])+","+str(self._stringLiberties[i])+') '
    ...
    def _draw_cross(self, x, y, w):
        toret = '<line x1="' + str(x - w) + '" y1="' + str(y) + '" x2="' + str(x + w) + '" y2="' + str(
            y) + '" stroke-width="3" stroke="black" />'
        toret += '<line x1="' + str(x) + '" y1="' + str(y - w) + '" x2="' + str(x) + '" y2="' + str(
            y + w) + '" stroke-width="3" stroke="black" />'
        return toret
    ...
    def svg(self):
        ...
        # The ABCD... line
        board += '<svg height="' + str(text_width) + '" width="' + str(
            text_width + border * 2 + (nb_cells - 1) * width) + '">'
        letters = "ABCDEFGHJ"
        il = 0
        ...
        # The line numbers
        il = 0
        board += '<svg width="' + str(text_width) + '" height="' + str(
            text_width + border * 2 + (nb_cells - 1) * width) + '">'
        ...
        board += ' <svg x="' + str(text_width) + '" y="' + str(text_width) + '" height="' + \
                 ...
                 '<rect x="0" y="0" width="' + str(width * (nb_cells - 1) + 2 * border) + '" height="' + str(
            width * (nb_cells - 1) + 2 * border) + '" fill="#B4927A" />\
        ...
        <line x1="' + str(
            border) + '" y1="' + wmax + '" x2="' + wmax + '" y2="' + wmax + '" stroke-width="4" stroke="black"/>'
        ...
        pieces = [(x, y, self._board[Board.flatten((x, y))]) for x in range(self._BOARDSIZE) for y in
                  range(self._BOARDSIZE) if
